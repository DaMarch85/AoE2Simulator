...
export function resolveScenario(draftInput: ScenarioDraft, ruleset: Ruleset): ResolvedScenario {
  ...
  if (needsRange && !hasBarracks && draft.assumptions.autoDefaults.inferHardPrereqs) {
    ...
    issues.push({
      ...
      suggestedPatch: "Replace the inferred Barracks with an explicit Dark Age Barracks event if you want tighter control.",
    });
  }
  ...
  if ((hasCastleQuestion || hasCastlePolicy) && !hasSecondFeudalBuilding) {
    issues.push({
      ...
      severity: "warning",
      message:
        "Castle Age requires a second Feudal building. The starter scaffold assumes a Blacksmith or Market will be added before Castle click.",
      suggestedPatch: "Add a Blacksmith or Market event explicitly.",
    });
  }

  const questions = draft.questions.length > 0 ? draft.questions : DEFAULT_QUESTIONS;
  const policies = mergePolicies(template?.policies ?? [], draft.policies);
